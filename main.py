...
def create_stage():
    ...
    try:
        ...
        if not data.get('name') or not data.get('expected_duration'):
            return jsonify({"error": "Nombre y duración esperada son obligatorios"}), 400
...
def create_substage():
    ...
    try:
        ...
        if not data.get('name') or not data.get('expected_duration') or not data.get('stage_id'):
            return jsonify({"error": "Nombre, duración esperada y etapa son obligatorios"}), 400
            ...
            if movement['type'] == 'uso' and movement.get('stage_id'):
                ...
                if stage:
                    ...
                    for product_data in movement['products']:
                        ...
                        if product:
                            product_name = product['name']
                            if product_name not in consumption[stage_name]:
                                consumption[stage_name][product_name] = 0
                            consumption[stage_name][product_name] += product_data['quantity']
        return jsonify(consumption)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
